...
2. **產品主線 Wave 2 續**：28 個 `@ignore` 等待實作（backlog→progress 只能由使用者晉升）。下一個：`02_RevokeKey.feature`「Secret Scanner 偵測到金鑰洩漏 — 批次自動撤銷」——**非 test-only 啟用，Wave 2 首個需要 production 開發的殘項**：批次撤銷命令（依 prefix 掃非終態金鑰）、reason 常值 `"Key leaked in public repository"`、「系統通知 Security Admin 和 Consumer」斷言落點未定（通知機制可能是新結構——spec 前先讀 design-doc／context-integration-spec 對應段與 api-spec 端點章節；若涉新 BC slice 或新依賴，依 Working Agreement 進 Plan Mode 求使用者核可，勿直接派工）。RevokeKeyHandler 剩餘未覆蓋 4%＝guard 1（KeyNotFound）＋successor 缺列分支，對應場景不在 02 檔內。派工一律用 `tasks/_templates/executor-spec.md`；spec 精度注意（累積）：測試計數勿外推（FunctionalTests dll = 46 場景＋5 hasher）、migration 需 `dotnet tool restore` 前置、帳面（bdd-progress）更新須排在 `ci-checks.sh fast` 之前。
