...
- **兩個 refactor 觀察點（Suspend 首場景 executor 申報，非阻塞）**：(1) production — Revoke/Suspend handler guard 鏈樣板相似，是否抽共用（orchestrator 傾向緩抽：兩例尚不足 rule-of-three，且 guard 語意各有差異）；(2) test — outbox 斷言樣板 Create/Revoke/Suspend 三處相似，已達三例，可考慮 test-only helper pass。
