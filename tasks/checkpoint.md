...
1. **產品主線**：35 個 `@ignore` BDD scenario 等待實作（backlog→progress 只能由使用者晉升）。下一個：`01_CreateApiKey.feature`「到期時間超過最大允許有效期 — 拒絕建立」（guard 5b `command.ExpiresAt > now.AddDays(ApiKey.GetMaxValidityDays())` 與 `WhenConsumerCreatesKeyExpiresIn5Years` step 皆已存在；該場景無任何 Given，When 接 `SeedDefaultPreconditionsIfMissingAsync`（已含 `any:read` 註冊，guard 4b 已無坑）即可，比照 9/44 pattern；spec 撰寫依新 lesson 沿 guard 鏈核對）。派工一律用 `tasks/_templates/executor-spec.md`。
