...
2. **產品主線 Wave 2 續**：31 個 `@ignore` 等待實作（backlog→progress 只能由使用者晉升）。下一個：`02_RevokeKey.feature`「從 Suspended 狀態撤銷」——形狀與 Locked 場景幾乎同構（test-only 啟用、seed 循 EF CurrentValue 先例、When 用既有「操作者撤銷」step），建議先落 `ee25317` trailer 登記的 seed-helper 重構（獨立 test-only pass，RevokeKeySteps 檔內；Suspended 將是第 5 份 `ApiKey.Create` 樣板，先抽再啟用最省）。派工一律用 `tasks/_templates/executor-spec.md`；spec 精度注意（累積）：測試計數勿外推（FunctionalTests dll = 46 場景＋5 hasher）、migration 需 `dotnet tool restore` 前置、步驟順序上帳面（bdd-progress）更新須排在 `ci-checks.sh fast` 之前（bdd-lint 會核帳面一致性）。RevokeKey guard 場景（未提供原因／終態）晉升可補 `RevokeKeyHandler` 失敗分支覆蓋（現 88% 未覆蓋處即三條 guard＋successor 缺列分支）。
