...
### 本機防線已上線（單一真相來源）
- [x] **正典閘 `scripts/ci-checks.sh`**：restore → build → format verify → test → adr-lint。pre-commit hook 與 `ci.yml` **都呼叫這支腳本**，兩道閘不可能漂移（commit `f621f61`）。
- [x] pre-commit hook 已安裝（`core.hooksPath = scripts/git-hooks/`）並實際觸發驗證——commit `f621f61` 自己跑過完整正典閘才放行。
- [x] 全套件 Green 經正典閘端到端確認（需 Docker 起，供 Testcontainers）。
