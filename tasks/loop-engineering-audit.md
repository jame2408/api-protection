...
### 待驗證（環境限制，非缺陷）
- [ ] CI 首次實跑：需 push 到 GitHub 後確認綠（本機已逐步驗證各步驟：format exit 0、adr-lint 6 檔過、test 全綠）
- [ ] `.github/workflows/ci.yml` 設為 main 的 required status check（需 repo 設定權限，人工於 GitHub 操作）
- [ ] pre-commit hook 安裝：`bash scripts/install-git-hooks.sh`（本機一次性）
