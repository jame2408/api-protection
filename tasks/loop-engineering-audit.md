...
### 第二批架構規則（2026-06-13，已落地）
- [x] **ILogger 邊界**：`LoggerBoundaryTests.cs`（reflection — Service/Domain/Handler 不得有 ILogger ctor 參數；Infrastructure ApiClient 與 Host 邊界豁免）
- [x] **命名**：`NamingConventionTests.cs`（reflection — `I*Handler`/`I*Repository` 實作必須對應命名，保護批1的 suffix scoping；`*FailureCodes` 必為 static class 且只含 const string）
- [x] **`new Failure(` / `cancel` 命名**（§5.4 決議：選 grep CI step，非 Roslyn analyzer）：`scripts/source-lint.sh`，接進 `ci-checks.sh`（fast+full 皆跑，cheap）。豁免 `FailureProvider.cs`。
- Architecture.Tests 由 3 → **11 tests**，全綠＋各故意紅驗證偵測有效。

### 寫的當下（write-time）防線已上線 — 四層 fail-fast 完整
- [x] **`bare-string failure code`**（§5.4）：`CreateFailure("FOO")` → `scripts/source-lint.sh` 新增一條 grep（現況 0 違規即綠）。
- [x] **PreToolUse hook**（§5.3，plan §B2）：`.claude/hooks/pre-tool-edit.py`，註冊於 `.claude/settings.json`（matcher `Edit|Write|MultiEdit`）。攔 `backend/src/**/*.cs` 寫入當下的 4 個高信心 pattern：`new Failure(`（豁免 FailureProvider）、bare-string code、`cancel` 命名、`ILogger<` 注入（Domain/Application/`*Handler.cs` 路徑）。**不納入 `throw`**（Result.cs 存取器、Infra 設定守衛、Contract 驗證都是合法 throw，會誤報；結構性違規由「Handler 必回 Result」測試覆蓋）。pattern 與 source-lint/架構測試一致 → 寫的當下與 commit/CI 不漂移。9 情境測試全對（違規 exit 2、豁免/邊界/範圍外 exit 0）。
- 防線層次現況：**寫的當下（PreToolUse hook）→ commit 前（pre-commit fast）→ push 前（pre-push full）→ CI**，最內層不再缺。

### Agent 啟動防線（plan §B）已完整
- [x] **B1 — session 啟動注入 must-read**：`session-init.sh` 在每個 session 首個 turn 注入必讀規範提示（references glob + ADR + api-spec + 指向 CLAUDE.md §0）。**刻意用 glob/指針而非硬編檔案清單** — 否則提醒本身會在新增規則檔時 drift（提醒自己變成死規則就是反模式）。提示並點明規則已被 hook/測試/CI 機械化強制，讓提醒有牙齒、可信。
- [x] **B2 — PreToolUse hook**（見上）。
- Agent 啟動防線兩件（B1 注入 + B2 攔截）皆上線。
