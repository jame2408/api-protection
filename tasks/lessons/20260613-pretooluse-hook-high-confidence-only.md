...
**落地:** `scripts/agent/hook.py` `pre-tool-edit` + `.claude/settings.json`／`.codex/hooks.json` PreToolUse wiring；exit 2 擋並回報，Claude／Codex fixtures 由 `scripts/hook-smoke.sh` 鎖定。
