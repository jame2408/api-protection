...
**落地:** 原「寫檔禁 heredoc、一律用 Write 工具」段已機械化 — `.claude/hooks/pre-tool-bash.py` heredoc 攔截（矩陣 23，commit `275e6ec`），2026-07-10 triage 自 Rule 行移除改防線代記；餘上列兩段為行為紀律。多行 commit message 改以 Write 寫訊息檔＋`git commit -F <file>`，不用 heredoc。
