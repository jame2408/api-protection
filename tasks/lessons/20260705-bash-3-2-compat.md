...
**落地:** `scripts/coverage-check.sh`、`scripts/ci-checks.sh` coverage 段（commit `e94a381`）；防線再加一層 `scripts/source-lint.sh` bash 相容段（掃 `scripts/**/*.sh` 禁 `mapfile`/`readarray`/`trap ... RETURN`，自身排除；hook 跨 harness 遷移見 ADR-023）。
