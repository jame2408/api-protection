...
|---|---|---|---|---|---|---|---|
| Claude Code CLI | ✅（本 audit session 即此 surface） | ✅ Agent tool `subagent_type`＋`.claude/agents/*.md` frontmatter 定義自訂角色 | ✅ 呼叫時 `model` 參數可逐次指定，agent 定義 frontmatter 可綁 model／reasoning | ✅ agent 定義 `tools` allowlist（內建 Explore 型即無 Edit／Write）＋`isolation: worktree` | 子代理無 Agent tool → 巢狀深度 1；可平行派發 | **model-routed** | 本 session Agent tool schema 機械觀察（2026-07-11）：`model` 參數存在且逐次可選；內建 read-only agent 型存在 |
...
| `.claude/agents/` 無 explorer／executor／reviewer 專案角色定義（現以內建 agent 型近似對映） | ADR-026 Phase 4（Claude adapter 對齊） |
