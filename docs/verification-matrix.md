...
| **`scripts/source-lint.sh` — 逐檢查段（7 個：C# method-body 語法 ×4、repo hygiene ×3，NetArchTest 與 reflection 都看不到）** ||||
...
| 9h | `generated_code = true` 僅允許出現在 Migrations 區段——整檔 analyzer 豁免只限工具產物，手寫檔不得標記 — `tasks/lessons/20260710-generated-code-marker-is-full-analyzer-exemption.md`（已歸檔，防線代記） | `scripts/source-lint.sh`（`bad_generated` 檢查段，掃描全部 tracked `.editorconfig`） | commit 前 / push 前 / CI | 腳本 |
...
| 19e | phase 收尾更新 `tasks/checkpoint.md` 前必跑 failure triage；`REPEAT` 簽名三選一處置（lesson / todo / checkpoint 記不轉理由）— `docs/adr/adr-018-failure-triage-and-observations-retirement.md` 決策 §3。報表末行同時機械判定 lessons triage 門檻（`tasks/lessons/` active ≥ 15 → 到期，`tasks/todo.md` 常設觸發條款的機械前哨；2026-07-10 巡檢根因補訊號） | `scripts/failure-triage.sh`（報表，含 `report_lessons_count` 段）＋人工判讀 | phase 收尾 | 人／大型模型 |
| **AI review 類** ||||
