...
本 repo 的真正產品是「多模型統一開發 loop」——API 金鑰管理服務只是載體。你扮演**規格擁有者與最終裁決者**；接手的大型模型扮演**協調者**（設計裁決、規格撰寫、review、派工）；實作由協調者派 executor 完成。任何常設大型模型都能只靠文件接手協調者角色（憲章 §1 明文規則 (ii)），不依賴特定模型在場——Fable 5 退場後照常運作。
