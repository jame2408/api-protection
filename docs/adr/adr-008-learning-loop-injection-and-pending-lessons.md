...
> **2026-07-05 修訂**（`docs/adr/adr-013-content-tiering-and-injection-slimming.md` 決策 (b)）：本條「最近 8 條全文」設計已被取代。`tasks/lessons.md` 改分 `## Active` / `## Archived（已機械化 — 防線代記）` 兩區；注入邏輯改為讀取 `## Active` 區**全部**條目的標題行 + `**Rule:**` 行（不含 Context / 落地），末尾附 Active／Archived 計數指針。不再需要數量上限 8——已機械化的教訓持續流向 Archived，Active 區規模本身會收斂。以下原文保留供歷史對照，現行規則以 ADR-013 決策 (b) 為準。
...
2. ~~注入內容固定為：must-read 段 + `tasks/lessons.md` 最近 8 條條目（以 `### [` 錨點切塊）+ 總數指針一行；上限 8 的變更須先開新 ADR。~~ **已由 `docs/adr/adr-013-content-tiering-and-injection-slimming.md` 決策 (b) 修訂**：注入內容為 must-read 段 + `tasks/lessons.md` `## Active` 區全部條目的標題行 + `**Rule:**` 行 + Active／Archived 計數指針一行；分區判準與歸檔規則見該 ADR。
