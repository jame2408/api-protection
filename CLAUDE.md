...
> Development phase only — `.feature` scenarios and API specs are already produced. 凍結的是 Discovery 新場景產出；範圍化解除逐案走 `docs/adr/adr-030-discovery-freeze-scoped-lift.md`（判準：API 契約已定案但場景從未產出；目前已解除 Data Plane）。既有場景修訂、缺陷再現、行為移除走 `docs/adr/adr-022-bdd-requirement-type-routing.md` 分流（§1 需求類型分流表）。
